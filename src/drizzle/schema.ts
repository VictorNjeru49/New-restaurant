...
export const authorityownertable = pgTable("authorityowner", {
  id: serial("id").primaryKey(),
  ownerId: integer("user_id").notNull().references(() => restaurant_ownertable.id),
  password: varchar("password", { length: 100 }),
  ownersname: varchar("username", { length: 100 }),
  role: roleEnum("role").default("restaurant_owner")
});
...
export const driverrelations = relations( authoritydrivertable, ({ one})=> ({
  driver: one(drivertable, {
    fields: [authoritydrivertable.driverId],
    references: [drivertable.id]
  })})) ;

  export const ownerrelations = relations( authorityownertable, ({ one })=> ({
    owner:one ( restaurant_ownertable,{
      fields:[authorityownertable.ownerId],
